...
PROMPTS = {
    ...
    "two_column_reading_order_reconstruction": (
        "你是期刊多栏版面阅读顺序恢复专家。请根据页面图片、文本块坐标和 OCR 文本恢复真实阅读顺序，"
        "区分页眉、页脚、页码、通栏区域、各个栏和跨栏图表。页面可能是单栏、双栏或三栏及以上，"
        "栏数以版面实际情况为准。"
        "论文首页如存在通栏摘要、关键词或元数据区域，必须先读取这些通栏内容；摘要下方进入多栏正文后，"
        "再按从左到右逐栏、每栏内部从上到下恢复正文。"
        "多栏正文必须按纵向带恢复：读取某一栏时若到达跨栏图表位置，必须先转读同一纵向带内右侧各栏的内容，"
        "不得继续读取跨栏图表下方本栏的内容；读完该带右侧各栏后，再读取跨栏图表，然后进入图表下方的新一带。"
        "生成的问题与答案必须忠于当前页面，答案要尽量覆盖该页所有可见正文、表格、图和公式。"
    ),
    "page_to_journal_layout_description": (
        "你是期刊页面结构描述专家。请判断页面类型、通栏区域、各栏区域（单栏、双栏或三栏及以上，以版面实际为准）、图表公式、页面主旨、"
    ...
    "domain_knowledge_corpus": """
你是领域期刊数据清洗与预训练语料构建助手。你的任务是把恢复多栏阅读顺序后的期刊论文文本处理成适合继续预训练的高质量文本语料。
