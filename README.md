...
栏数由数据决定，不再假设两栏。`_column_bands()` 把"窄块"的水平范围投影到 x 轴，连续被覆盖的区间就是一栏，区间之间的空白就是栏间距。

- 两栏沿用 `left` / `right` 标签，下游逻辑和导出标签不变；三栏以上用 `col1`…`colN`，并写入 `block.column_index`（0 起，跨栏/通栏为 -1）。
- 跨栏块按"压到几条栏"分类：压到全部 → `full_width`，压到多条但非全部 → `cross_column`，压到一条 → `single_column`。
- 读序按纵向带恢复：带内从左到右逐栏、栏内从上到下，跨栏块结束当前带。
- **多阈值扫描**：单一阈值分不开「0.57 宽的非对称正文栏」和「0.65 宽的两栏跨图」。因为跨栏块只会把相邻栏粘成一条、永远不会把一栏劈开，所以按 `layout.narrow_scan_ratios` 各扫一遍取栏数最多的结果。
- 整版图片页找不到栏时退回单栏，不会变成 `unknown`。

在《航空知识》真实扫描页上验证过 14 页（单栏/两栏/非对称两栏/三栏/整版图），栏数判定全部一致。

## 噪声页与卖家水印

买来的整期扫描件夹带订阅广告、杂志社声明、二维码推广页，而且**每页页脚都印着一行卖家微信**，OCR 后会混进每一页正文。

- `page_noise.strong_patterns`：命中任意一条即判定（`PDF过刊`、`杂志收藏购买`、`bfwz\d+` 等，正经文章不会出现）。命中的**块**标成 `watermark`，被 `_skip_from_train_text` 和 routing 的 `skip_roles` 一起剔除；命中的**页**直接判 `advertisement_or_notice`。
- `page_noise.patterns`：多特征累计判定，短页需 `min_signals`（2）、长页需 `strong_signals`（4），避免正文偶尔提到「网址」「微信」被误杀。
- `advertisement_or_notice` 本来就在 `low_value_page_types` 里，但改造前 `_page_type()` 从不返回这个值 —— 过滤器一直是空转的。
